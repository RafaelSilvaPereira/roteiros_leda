...
public class BSTImpl<T extends Comparable<T>> implements BST<T> {
	...
	private int height(BSTNode<T> nodeAt) {
		int height = 0;
		if (!nodeAt.isEmpty()) {
	...
	private BSTNode<T> sucessor(BSTNode<T> nodeAt, BSTNode<T> nodeFrom) {
		...
		if (!nodeAt.isEmpty()) {
			...
			if (isRightEligible) {
				...
				int comparation = nodeAt.getParent().getData()
						.compareTo(nodeAt.getData());
				if (comparation == GREATER)
	...
	private BSTNode<T> predecessor(BSTNode<T> nodeAt, BSTNode<T> from) {
		BSTNode<T> result = null;
		if (!nodeAt.isEmpty()) {
			boolean isLeftEligible = !nodeAt.getLeft().isEmpty()
					&& !nodeAt.getLeft().getData().equals(from.getData());
			if (isLeftEligible) {
	...
	private void remove(BSTNode<T> nodeAt) {
		if (!nodeAt.isEmpty()) {
			if (nodeAt.getLeft().isEmpty() || nodeAt.getRight().isEmpty()) {
				...
				BSTNode<T> newThis = sucessor(nodeAt, nodeAt);
				nodeAt.setData(newThis.getData());
				remove(newThis);
			}
		}
	...
	private BSTNode<T> getParent(BSTNode<T> nodeAt) {
